...
// ==========================================
// 4. LOGIKA INTEGRASI PENCARIAN & SCROLL OTOMATIS
// ==========================================
...
// B. Aksi saat Menekan Tombol Enter: Tutup Search & Arahkan Layar ke Produk Pertama yang Cocok
searchInput.addEventListener('keydown', function (e) {
    if (e.key === 'Enter') {
        ...
        // Cari kartu produk pertama yang berstatus tampil (display !== 'none') dan cocok dengan kata kunci
        productCards.forEach(card => {
            const title = card.querySelector('.products-card-title').textContent.toLowerCase();
            if (title.includes(keyword) && card.style.display !== 'none' && !targetCard) {
                targetCard = card; // Kunci kartu pertama yang ditemukan
            }
        });

        // 1. Tutup overlay pencarian hitam secara otomatis
        searchForm.classList.remove('active');

        // 2. Arahkan layar ke produk yang dicari
        if (targetCard) {
            // Jika produk yang dicari ketemu, layar langsung turun dan fokus tepat ke kartu produk tersebut
            setTimeout(() => {
                targetCard.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'center' });
            }, 300); // Diberi jeda 300ms agar transisi penutupan search form selesai dulu
        } else if (productsSection) {
            // Jika tidak ada produk yang cocok, layar diarahkan ke awal section produk secara umum
            setTimeout(() => {
                productsSection.scrollIntoView({ behavior: 'smooth', block: 'center' });
            }, 300);
        }

        // Hapus teks pencarian agar bersih kembali saat dibuka nanti
        this.value = '';
    }
});
...
function addToCart(product) {
    ...
    // Animasi Detak Mikro Umpan Balik Sukses
